...
public class VendaControle {
	...
	public ResponseEntity<List<Venda>> buscarVendas() {
		// Busca todas as vendas do repositório
	...
	public ResponseEntity<Venda> buscarVenda(@PathVariable Long id) {
		// Busca a venda pelo ID fornecido
			...
			if(dados.getIdMercadorias() != null) {
				if(!dados.getIdMercadorias().isEmpty()) {	
					for (Long id : idsMercadorias) {
						Mercadoria respostar = repositorioMercadoria.getById(id);
						Mercadoria mercadoria = new Mercadoria();
						mercadoria.setValidade(respostar.getValidade());
						mercadoria.setFabricao(respostar.getFabricao());
						mercadoria.setCadastro(respostar.getCadastro());
						mercadoria.setNome(respostar.getNome());
						mercadoria.setQuantidade(respostar.getQuantidade());
						mercadoria.setValor(respostar.getValor());
						mercadoria.setDescricao(respostar.getDescricao());
			...
			if(dados.getIdServicos() != null) {	
				if(!dados.getIdServicos().isEmpty()) {				
					for (Long id : idsServicos) {
						Servico resposta = repositorioServico.getById(id);
						Servico servico = new Servico();
						servico.setNome(resposta.getNome());
						servico.setDescricao(resposta.getDescricao());
						servico.setValor(resposta.getValor());
	...
	public ResponseEntity<?> atualizarVenda(@PathVariable Long idVenda, @RequestBody Venda dados) {
		// Atualiza a venda com os dados fornecidos
	...
	public ResponseEntity<?> excluirVenda(@PathVariable Long idVenda) {
		// Exclui a venda com base no ID fornecido
